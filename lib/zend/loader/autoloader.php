 ...
 *
 * @category Zend
 * @package mail_booster_Zend_Loader
 ...
 * @uses mail_booster_Zend_Loader_Autoloader
 * @package mail_booster_Zend_Loader
 * @subpackage Autoloader
 * @copyright  Copyright (c) 2005-2015 Zend Technologies USA Inc. (http://www.zend.com)
 * @license http://framework.zend.com/license/new-bsd	  New BSD License
 */
class mail_booster_Zend_Loader_Autoloader {

    /**
     * @var mail_booster_Zend_Loader_Autoloader Singleton instance
     */
     ...
     */
    protected $_defaultAutoloader = array('mail_booster_Zend_Loader', 'loadClass');
     ...
     *
     * @return mail_booster_Zend_Loader_Autoloader
     */
    ...
    public static function autoload($class) {
        ...
        foreach ($self->getClassAutoloaders($class) as $autoloader) {
            if ($autoloader instanceof mail_booster_Zend_Loader_Autoloader_Interface) {
    ...
    public function setDefaultAutoloader($callback) {
        if (!is_callable($callback)) {
            throw new mail_booster_Zend_Loader_Exception('Invalid callback specified for default autoloader');
        }
     ...
     *
     * @param  array $autoloaders Array of PHP callbacks (or mail_booster_Zend_Loader_Autoloader_Interface implementations) to act as autoloaders
     * @return mail_booster_Zend_Loader_Autoloader
     */
    ...
    public function registerNamespace($namespace) {
        if (is_string($namespace)) {
            ...
            throw new mail_booster_Zend_Loader_Exception('Invalid namespace provided');
        }
    ...
    public function setZfPath($spec, $version = 'latest') {
        ...
        if (is_array($spec)) {
            if (!isset($spec['path'])) {
                throw new mail_booster_Zend_Loader_Exception('No path specified for ZF');
            }
            $path = $spec['path'];
     ...
     * @return bool|mail_booster_Zend_Loader_Autoloader Returns boolean if no argument is passed, object instance otherwise
     */
     ...
     *
     * @param  object|array|string $callback PHP callback or mail_booster_Zend_Loader_Autoloader_Interface implementation
    ...
    protected function _autoload($class) {
        ...
        try {
            ...
            return $class;
        } catch (mail_booster_Zend_Exception $e) {
            return false;
        }
    }
    ...
    protected function _getVersionPath($path, $version) {
        ...
        if (empty($availableVersions)) {
            throw new mail_booster_Zend_Loader_Exception('No valid ZF installations discovered');
        }
     ...
     * @throws mail_booster_Zend_Loader_Exception if version string contains too many dots
     */
    protected function _getVersionType($version) {
        ...
        if (3 < $count) {
            throw new mail_booster_Zend_Loader_Exception('Invalid version string provided');
        }
        return 'specific';
    }
    ...
    protected function _getAvailableVersions($path, $version) {
        if (!is_dir($path)) {
            throw new mail_booster_Zend_Loader_Exception('Invalid ZF path provided');
        }
