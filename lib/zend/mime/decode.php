 ...
 * @license http://framework.zend.com/license/new-bsd New BSD License
 */
class mail_booster_Zend_Mime_Decode {
    ...
    $message, &$headers, &$body, $EOL = mail_booster_Zend_Mime::LINEEND
    ) {
    ...
    $field, $wantedPart = null, $firstName = 0
        ...
        if (!preg_match_all('%([^=\s]+)\s*=\s*("[^"]+"|[^;]+)(;\s*|$)%', $field, $matches)) {
            throw new mail_booster_Zend_Exception('not a valid header field');
        }
