...
if (!class_exists("mail_booster_manage_email")) {
    if (file_exists(MAIL_BOOSTER_DIR_PATH . "includes/mail-booster-manage-email-address.php")) {
        require_once MAIL_BOOSTER_DIR_PATH . "includes/mail-booster-manage-email-address.php";
    }

    class mail_booster_manage_email {
        ...
        // 	if($inst == null)
        // 	{
        // 		$inst = new mail_booster_manage_email();
        // 	}
        // 	return $inst;
        // }

        public function check_email_body_parts_mail_booster() {
            return empty($this->body_textpart) && empty($this->body_htmlpart);
        }

        public function validate_email_contents_mail_booster($transport) {
            $this->validate_email_headers_mail_booster();
        }
        ...
        public function validate_email_headers_mail_booster() {
            if (isset($this->reply_to)) {
                $this->mb_get_reply_to()->validate_email_contents_mail_booster("Reply-To");
            }

            // validate the from address
            $this->get_email_address_mail_booster()->validate_email_contents_mail_booster("From");

            // validate the to recipients
            foreach ((array) $this->mb_get_to_recipients() as $to_address) {
                $to_address->validate_email_contents_mail_booster("To");
            }

            // validate the cc recipients
            foreach ((array) $this->mb_get_cc_recipients() as $cc_address) {
                $cc_address->validate_email_contents_mail_booster("Cc");
            }

            // validate the bcc recipients
            foreach ((array) $this->mb_get_bcc_recipients() as $bcc_address) {
                $bcc_address->validate_email_contents_mail_booster("Bcc");
            }
        }

        public function get_email_address_mail_booster() {
            return $this->from;
        }
        ...
        public function mb_add_recipients(&$all_recipients, $recipients) {
            if (!empty($recipients)) {
                $recipients = mail_booster_manage_email_address::convert_string_to_array_mail_booster($recipients);
                foreach ($recipients as $recipient) {
                    if (!empty($recipient)) {
                        array_push($all_recipients, new mail_booster_manage_email_address($recipient));
                    }
                }
            }
        }
        ...
        public function mb_add_attachments_to_mail(mail_booster_Zend_Mail $mail) {
            ...
            foreach ($attributes_array as $file) {
                if (file_exists($file)) {
                    $at = new mail_booster_Zend_Mime_Part(file_get_contents($file));
                    $at->disposition = mail_booster_Zend_Mime::DISPOSITION_ATTACHMENT;
                    $at->encoding = mail_booster_Zend_Mime::ENCODING_BASE64;
        ...
        function mb_set_from($email, $name = null) {
            if (!empty($email)) {
                $this->from = new mail_booster_manage_email_address($email, $name);
            }
        }

        function mb_set_replyto($reply_to) {
            if (!empty($reply_to)) {
                $this->reply_to = new mail_booster_manage_email_address($reply_to);
            }
        }
