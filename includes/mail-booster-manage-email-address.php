...
if (!class_exists("mail_booster_manage_email_address")) {

    class mail_booster_manage_email_address {
        ...
        public function validate_email_contents_mail_booster($description = "") {
            if (!mail_booster_zend_mail_helper::email_validation_mail_booster($this->email)) {
        ...
        public static function convert_string_to_array_mail_booster($emails) {
            if (!is_array($emails)) {
                $t = explode(",", $emails);
                $emails = array();
                foreach ($t as $k => $v) {
